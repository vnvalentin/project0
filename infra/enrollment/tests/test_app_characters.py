...
def _client(character_client) -> TestClient:
    return TestClient(create_app(FakeLoginAuthorityClient(), character_client))


def test_list_returns_characters():
...
def test_create_returns_character():
    client = _client(FakeCharacterClient(created_character={"character_id": "c9", "display_name": "Hero"}))
...
def test_create_name_taken_maps_to_409():
    client = _client(FakeCharacterClient(fail_with_reason="NAME_TAKEN"))
...
def test_select_returns_character_assertion():
    client = _client(FakeCharacterClient(selected_assertion="character-token"))
...
def test_select_unknown_character_maps_to_404():
    client = _client(FakeCharacterClient(fail_with_reason="NO_SUCH_CHARACTER"))
...
def test_delete_ok():
    client = _client(FakeCharacterClient())
...
def test_bad_assertion_maps_to_401():
    client = _client(FakeCharacterClient(fail_with_reason=CharacterAuthorityError.ASSERTION_REJECTED))
...
def test_missing_assertion_rejected_by_validation():
    client = _client(FakeCharacterClient())
...
def test_character_route_unavailable_when_client_not_wired():
