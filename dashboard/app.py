...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path == "/health":
            ...
            self.send_response(200)
            self.send_header("Content-Type", "text/html; charset=utf-8")
        elif path == "/prototype-roadmap":  # PROTOTYPE, issue #380 -- throwaway
            variant = parse_qs(parsed.query).get("variant", ["A"])[0].strip().upper()
            body = render_prototype_roadmap(variant).encode("utf-8")
            self.send_response(200)
            self.send_header("Content-Type", "text/html; charset=utf-8")
        elif path == "/telemetry":
