...
def page() -> str:
    ...
    goal_cards = "".join(
        ...
        f'<div class="goal-top"><span class="goal-id">GOAL {letter}</span><span class="state">{esc(status.upper())}</span></div>'
        f'<h3>{esc(title)}</h3><p>{esc(description)}</p>'
        f'<div class="goal-foot"><span>{"Next design gate" if status in {"chartering", "design gate", "decision-gated", "research-first"} else "Delivery track"}</span>'
