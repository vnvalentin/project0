...
- Current slice: [142 — Phase 15 follow-on (P-016): live RPC replication of the EffectiveMechanicsSnapshot](slices/142-effective-mechanics-replication.md) — **delivered; the post-exit-gate follow-on Slice 140 deferred — the server derives an `EffectiveMechanicsSnapshot` at world entry and replicates its presentation-safe view to the owning client (mirroring the Phase 14 Character-snapshot channel), which validates it fail-closed and renders a HUD readout; new `test_effective_mechanics_replication` 7/7 + `test_effective_mechanics_label` 5/5; full GUT suite 723/723 across 99/99, exit 0 on the Linux host**. Prior: [Slice 140](slices/140-phase15-embodiment-progression-service.md) closed the exit gate; Slices 132-134 (P-016-A foundation), 135-139 (subsystems B–F). Outstanding follow-on: vessel SQLite persistence.
- Tech debt: none.
