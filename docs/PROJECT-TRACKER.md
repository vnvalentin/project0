...
- **Current slice:** [079 — Optional dedicated Canon store (opt-in canon/accounts DB split)](slices/079-optional-dedicated-canon-store.md) — **delivered; game server opens a dedicated Canon SqliteStore when PROJECT0_CANON_DB_PATH is set (unset preserves the Slice 045 shared handle, no migration); fail-closed on open failure; GUT 58/58 and runtime boot (split names split_canon.db + both files created; unset shared) passed on Linux**
