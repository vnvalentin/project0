...
- **Current slice:** [081 — Deploy the standalone login server via docker-compose (opt-in profile)](slices/081-deploy-login-server-compose.md) — **delivered; deploy/game-server/docker-compose.yml adds a profile-gated `login-server` service (reuses the game image, entrypoint override runs login_server_main.gd on its own port/DB/health file) plus a shared empty-default PROJECT0_ASSERTION_SECRET on both; default `docker compose up` unchanged; validated on Linux — compose config (default lists only game-server; login-split adds login-server) and a real container run reaching Docker health `healthy`, GUT 58/58 unaffected**
