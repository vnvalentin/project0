...
- **Current slice:** [080 — One-time Canon migration into a dedicated store on first split boot](slices/080-canon-migration-on-split-boot.md) — **delivered; CanonRepository.list_all_records/restore_record and server_main copy Canon out of the shared accounts store into a newly-empty dedicated canon store on first split boot (preserving created_at, source never deleted, re-boot skips); GUT 58/58 and a three-boot runtime migration check passed on Linux**
