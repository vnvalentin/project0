...
- **Current slice:** [084 — Login-split cutover: split on by default](slices/084-login-split-cutover.md) — **delivered; shared/network_config.gd client_login_split_enabled() now defaults ON (opt out with PROJECT0_CLIENT_LOGIN_SPLIT=0) and server/server_main.gd runs assertion-only by default (opt out with PROJECT0_GAME_ASSERTION_ONLY=0); the login split is now canonical; proven on Linux — GUT 58/58, default boot logs assertion-only mode: true (=0 logs false), and the login handoff e2e is ALL PASS with the new default (world_entry ok, "Handoff Hero")**
