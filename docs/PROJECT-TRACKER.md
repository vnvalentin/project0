...
| 12. Authoritative runtime and action input | in-progress | Production deployment mutation and rollback evidence are recorded, login and game images have an explicit release boundary, and the server resolves a bounded action set authoritatively beyond the first melee seam. [Slice #533](https://github.com/vnvalentin/project0/issues/533) excludes admitted but unbound peers from shared-world presence. |
...
- **Current slice:** [#533 — Exclude unbound peers from shared-world presence](https://github.com/vnvalentin/project0/issues/533) — **delivered; presence requires server-bound Account and Character identity, while ENet/version admission and Player replication remain unchanged; full Linux GUT passes 885/885.**
...
- Current slice: [157 — Phase 16 (F-037): visible WAN package and opt-in stable-directory updates](slices/157-visible-wan-package.md) — **in progress; package boot, first install, failed-readiness rollback, live signed download/staging, and bounded headless readiness are implemented; reliable valid-update launcher observation remains**. Prior: [155](slices/155-outdated-client-launcher-handoff.md).
- Tech debt: none.
...
Progress: **100%** (7 of 7 planned slices delivered; bounded v1 read/control proof passed on `okami`)
...
- Feature: `implemented` [F-041](FEATURE-LIST.md#f-041-fleet-operations-console) — Slices 184-190 delivered with bounded v1 read/control proof.
- Current slices: [184 — Server admin console OpsSnapshot contract](slices/184-ops-snapshot-contract.md) — **delivered; pure server-owned contract with focused GUT coverage.** [185 — Atomic OpsSnapshot writer](slices/185-ops-snapshot-writer.md) — **delivered; atomic temporary-write/publish seam with focused HealthReporter coverage.** [186 — OpsSnapshot registry and freshness](slices/186-ops-registry-freshness.md) — **delivered; deterministic manifest scanner with healthy/stale/absent/unreadable classification.** [187 — Read-only operator console surface](slices/187-operator-console-read-only-surface.md) — **delivered; LAN-bound stdlib HTTP fleet/detail surface with read-only mounts.** [188 — Control contracts](slices/188-control-contract.md) — **delivered; bounded action/result/audit contracts with focused pytest coverage.** [189 — Operator assertion seam](slices/189-operator-auth-seam.md) — **delivered; independent assertion verification, fixed-allowlist host helper, authoritative Godot control seam, and live rejection/authorization proof.** [190 — Operator console proof/runbook](slices/190-operator-console-proof-runbook.md) — **delivered; live world/login snapshots, healthy LAN console rows, and reversible authorized control proof passed.**
...
- Current slice: [191 — Normalized World directive validator](slices/191-world-directive-validator.md) — **delivered; pure bounded validator and deterministic fallback implemented with focused GUT coverage.** [192 — Deterministic World directive ordering and replay identity](slices/192-world-directive-ordering.md) — **delivered; pure deterministic ordering and replay identity seam with focused GUT coverage.** [193 — Deterministic POI feasibility selection](slices/193-world-poi-feasibility.md) — **delivered; pure predicate feasibility and deterministic candidate selection with focused GUT coverage.** [194 — Deterministic builder feasibility contract](slices/194-world-builder-feasibility.md) — **delivered; bounded placement feasibility and whole-proposal fallback with focused GUT coverage.**
...
- **Slice:** [#533 — Exclude unbound peers from shared-world presence](https://github.com/vnvalentin/project0/issues/533) — **delivered; `NakamaPresence.bound_entry` excludes incomplete server-owned identity, Character binding republishes presence, focused coverage passes 5/5 with 10 assertions, the real socket harness prints `ALL PASS` with zero malformed-presence warnings, and full Linux GUT passes 126/126 scripts, 885/885 tests, 2,741 assertions, exit 0.**
  - **Feature:** [#457 — Character world entry](https://github.com/vnvalentin/project0/issues/457)
  - **Public seam:** `server/server_main.gd::_broadcast_presence_snapshot` output validated by `shared/nakama_presence.gd`
  - **Decision:** no new ADR; preserves the existing distinction between connection admission and Character world entry
  - **Root-cause learning:** admission-time Player state existed before Account/Character binding, so the broadcaster emitted fallback/empty identity that the strict presence contract correctly rejected; typed test fixtures now ensure the validator assertions execute
...
- [x] Delivered — Exclude admitted but unbound peers from shared-world
  presence (Phase 12, [Slice #533](https://github.com/vnvalentin/project0/issues/533),
  [Feature #457](https://github.com/vnvalentin/project0/issues/457)); preserve
  ENet/version admission and Player replication while requiring server-bound
  Account and Character identity for presence entries; focused, real-socket,
  and full Linux GUT evidence is green.
