...
- **Current slice:** [089 — Auth-gated onboarding B: `/redeem` signed-assertion + idempotent per-account peer lifecycle](slices/089-auth-gated-onboarding-peer-provisioning.md) — **delivered; validated on the Linux host (loopback POST /internal/validate-assertion + LoginGateway.validate_assertion; enrollment /redeem assertion path with per-account idempotency, key-mismatch rejection, CLI deprovision-stale aging, nullable-invite_code migration); GUT 420/420 across 62/62 scripts (1553 asserts), exit 0; enrollment pytest 96/96, exit 0 (also reproduced on Windows)**
