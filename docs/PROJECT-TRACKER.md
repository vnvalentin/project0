...
- **Current slice:** [083 — One-command split launcher with shared-secret management](slices/083-split-launcher-shared-secret.md) — **delivered; deploy/game-server/run-split.sh requires-or-generates a shared PROJECT0_ASSERTION_SECRET and brings up the split topology (base + split overlay + login-split profile), waiting for both containers healthy; `down` tears it down; no code default changed so single-process dev is untouched; proven on Linux — up → both healthy, handoff world_entry: ok ("Handoff Hero"), down clean; GUT 58/58 unaffected**
