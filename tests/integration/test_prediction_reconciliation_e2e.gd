...
## and tears down its own real server process bound to 127.0.0.1:9999. This
...
func test_prediction_reconciliation_harness_passes() -> void:
	...
	var exit_code: int = OS.execute(
		...
		PackedStringArray([
			...
			"-s", "scripts/test_prediction_reconciliation.gd",
		]),
		output,
		true,
	)
	...
	assert_eq(exit_code, 0, "prediction/reconciliation E2E harness exits 0 — output tail:\n%s" % _tail(joined_output))
